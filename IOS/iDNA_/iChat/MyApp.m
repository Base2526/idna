...
-(void)viewDidDisappear:(BOOL)animated{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"Tab_iDNA_reloadData" object:nil];
...
//            }
            ...
            */
            ...
            NSArray* myApp = [myAppRepo get:app_id];
            ...
            if ([f objectForKey:@"image_url"]) {
                ...
                [view.hjmPhoto setUrl:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@", [Configs sharedInstance].API_URL, [f objectForKey:@"image_url"]]]];
                [[(AppDelegate*)[[UIApplication sharedApplication] delegate] obj_Manager ] manage:view.hjmPhoto ];
            }else{
                [view.hjmPhoto clear];
            }
            
            [view.labelName setText:[f objectForKey:@"name"]];
            ...
            if ([[[Configs sharedInstance] getUIDU] isEqualToString:[f objectForKey:@"owner_id"]]) {
                ...
                if([f objectForKey:@"followers"]){
                    NSDictionary *followers =[f objectForKey:@"followers"];
                    
                    int count =0;
                    for (NSString* key in followers) {
                        NSDictionary* value = [followers objectForKey:key];
