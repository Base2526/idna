...
//@implementation UpdateMyAppThread
//
//@end
...
-(void)start: (UIImage *)img :(NSString *)strName: (NSString *)strPhoneNumber :(NSString *) strLocation :(NSString *)strGooglePlus :(NSString *)strFacebook
{
    ...
    NSURL *url = @"";//[NSURL URLWithString:[NSString stringWithFormat:@"%@%@.json",  [Configs sharedInstance].API_URL, [Configs sharedInstance].UPDATE_MY_APP ]];
    ...
    NSString *dataToSend = [[NSString alloc] initWithFormat:@"uid=%@&image=%@&name=%@&phone_number=%@&location=%@&google_plus=%@&facebook=%@", [[Configs sharedInstance] getUIDU],imgString, strName, strPhoneNumber, strLocation, strGooglePlus, strFacebook];
