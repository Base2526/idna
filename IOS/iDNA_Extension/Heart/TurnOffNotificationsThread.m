//
//  TurnOffNotificationsThread.m
//  Heart
//
//  Created by Somkid on 12/25/2559 BE.
...
@implementation TurnOffNotificationsThread
...
-(void)start: (NSString *)uid_friend: (NSString *)status
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@.json",  [Configs sharedInstance].API_URL, [Configs sharedInstance].TURN_OFF_NOTIFICATION]];
    ...
    NSString *dataToSend = [[NSString alloc] initWithFormat:@"uid=%@&uid_friend=%@&status=%@", [[Configs sharedInstance] getUIDU], uid_friend, status];
